...
Cloud computing is currently a major thread trend in the IT culture, companies
...
the overall quality of the hosting is in most of the case better that what can
ben done by the customers directly. It is not their job, they don't have as
much knowleldge as the provider which is specialized in this field.

Behind the user perspective, from the provider point of view, they own the
infrastructure and have to allocate resources from it for their customers.  It
is composed of a certain amount of physical machines (PMs) which could be
dispatched among different data centers, and each of these PMs, contains a
variable number of virtual machines (VMs), then is the \textbf{PaaS}, each of
them are running a set containerized applications. The problematic which is now
...
Using Online Bin Packing}~\cite{reassignmentVisbp}, the authors also
...
We have seen that cloud computing is a hot topic in the Internet industry which
results in a lot of new problematics in computer science. The resource
...
algorithms listed in this document gather publications around the virtual
machine assignment and reassignment on a set of physical machines.

Whatever is the kind of process used to solve this problem, the goal is always
the identical. Given a set of resources with a given capacity, how to choose
the best position for a task which has to consume those resources.

In the first part, a general solution method is studied. Using Linear
Programming, we are able to get the optimized result. However as this problem
...
overview of other methods like the Ant Colony heuristics, or network flows
based algorithms will be proposed.
